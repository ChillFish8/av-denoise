...
/// Applies the forward and backward neighbour contributions at every
/// pixel from a weight map.
///
/// `weights_fwd` and `weights_bwd` may be the same buffer, which is what
/// the symmetric case at temporal offset 0 does.
///
...
///
/// The result is `(original * m + acc) / (m + weight_sum)`, where `m` is
/// `wref * max_weight`.
///
/// When the denominator is close to zero, meaning no usable match was
/// found anywhere in the search window, the original pixel is kept as it
/// is.
///
/// The centre pixel is read from `input`'s frame `center_frame` and the
/// result written to `output`'s frame `output_frame`. A caller writing
/// into a ring buffer slot therefore passes the whole ring, rather than
/// binding it at the slot's byte offset, which the GPU only accepts on
/// its own alignment boundaries.
#[cube(launch_unchecked)]
pub fn nlm_finish<N: Size>(
    ...
    // `Vector::empty` zeroes its lanes, so the padding lanes a 3-channel
    // frame gets in 4-lane storage stay 0 whichever branch runs below.
