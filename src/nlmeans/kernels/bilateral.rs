...
/// a `(block + 2·radius)²` tile of source pixels into shared memory,
/// then convolves over the patch using
...
///
/// Channels are spread across `stored_ch` scalar shared-memory planes
/// rather than a single `Line<f32>` plane — vectorised SMEM with
/// runtime line size hits a cubecl-opt assertion on some backends, so
/// the per-channel layout sidesteps it without changing SMEM volume.
#[cube(launch)]
pub fn nlm_bilateral(
    ...
    let mut smem = SharedMemory::<f32>::new(comptime!(
        (block_x + 2 * radius) * (block_y + 2 * radius) * stored_ch
    ) as usize);
    ...
    if interior {
        while idx < tile_elems {
            ...
            let pixel = read_line(input, src_x, src_y, frame, width, height);
            let base = (idx * stored_ch) as usize;
            #[unroll]
            for c in 0..stored_ch {
                smem[base + c as usize] = pixel[c as usize];
            }
            idx += threads;
        }
    } else {
        while idx < tile_elems {
            ...
            let pixel = read_clamped_line(input, src_x, src_y, frame, width, height);
            let base = (idx * stored_ch) as usize;
            #[unroll]
            for c in 0..stored_ch {
                smem[base + c as usize] = pixel[c as usize];
    ...
    let center_base = ((center_tile_y * tile_width + center_tile_x) * stored_ch) as usize;
    ...
    for offset_y in 0..patch_size {
        for offset_x in 0..patch_size {
            ...
            let smem_base = (((center_tile_y + offset_y - radius) * tile_width + center_tile_x + offset_x
                - radius)
                * stored_ch) as usize;
            ...
            #[unroll]
            for c in 0..channels {
                let d = smem[smem_base + c as usize] - smem[center_base + c as usize];
                range_sq += d * d;
            }
            let spatial = (dx * dx + dy * dy) as f32 * inv_two_sigma_s_sq;
            let w = f32::exp(-(spatial + range_sq * inv_two_sigma_r_sq));

            let mut neighbor = Line::<f32>::empty(input.line_size()).fill(0.0f32);
            #[unroll]
            for c in 0..stored_ch {
                neighbor[c as usize] = smem[smem_base + c as usize];
            }
